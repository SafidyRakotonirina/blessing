  ...
  static async createComplete(inscriptionData) {
    ...
    try {
      ...
      const [niveauRows] = await connection.execute(
        `SELECT n.frais_inscription, n.frais_ecolage, n.prix_livre
         FROM vagues v
      ...
      const { frais_inscription, frais_ecolage, prix_livre } = niveauRows[0];
      
      // Calculer montants
      const montant_total_ecolage = parseFloat(frais_inscription) + parseFloat(frais_ecolage);
      const montant_paye_ecolage = (frais_inscription_paye ? parseFloat(frais_inscription) : 0) + parseFloat(montant_ecolage_initial);
      ...
      await connection.execute(
        `INSERT INTO ecolages (inscription_id, montant_total, montant_paye, montant_restant, frais_inscription_paye, statut_ecolage)
         VALUES (?, ?, ?, ?, ?, ?)`,
        [inscriptionId, montant_total_ecolage, montant_paye_ecolage, montant_restant, frais_inscription_paye, statut_ecolage]
      );
      ...
      for (let i = 1; i <= 2; i++) {
        ...
        await connection.execute(
          `INSERT INTO livres (inscription_id, numero_livre, prix, statut_paiement, statut_livraison)
           VALUES (?, ?, ?, ?, 'non_livre')`,
          [inscriptionId, i, prix_livre, livre_paye ? 'paye' : 'non_paye']
        );
      }
      ...
      if (livre1_paye) {
        await connection.execute(
          ...
          [inscriptionId, prix_livre, date_inscription]
        );
      }
  ...
  static async findById(id) {
    const [rows] = await pool.execute(
      `SELECT i.*,
              ...
              ec.frais_inscription_paye, ec.statut_ecolage
       FROM inscriptions i
  ...
  static async findByEtudiant(etudiantId) {
    const [rows] = await pool.execute(
      `SELECT i.*,
              ...
              n.code as niveau_code, n.nom as niveau_nom,
              s.nom as salle_nom,
              ec.montant_total, ec.montant_paye, ec.montant_restant, ec.statut_ecolage,
    ...
    try {
      ...
      const [paiementResult] = await connection.execute(
        `INSERT INTO paiements (inscription_id, type_paiement, montant, date_paiement, methode_paiement, reference, remarques, utilisateur_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [inscription_id, type_paiement, montant, date_paiement, methode_paiement, reference, remarques, utilisateur_id]
      );
      ...
      if (type_paiement === 'inscription' || type_paiement === 'ecolage') {
        ...
        await connection.execute(
          'UPDATE ecolages SET statut_ecolage = ? WHERE inscription_id = ?',
